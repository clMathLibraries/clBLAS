	...
	bool &specialCaseHandled)
{
    ...
    if (gfxipMajor == GFXIP_MAJOR_VEGA)
	{
#if 1
		if (order == clblasColumnMajor)
		{
			...
			{
				if (0) {}
				else if ((M % 64) != 0 && (N % 64) != 0 && (K % 64) != 0) /* MIXED */	{}
				else if ((M % 64) == 0 && (N % 64) != 0 && (K % 64) != 0) /* M64 */		{}
				else if ((M % 64) != 0 && (N % 64) == 0 && (K % 64) != 0) /* N64 */		{}
				else if ((M % 64) != 0 && (N % 64) != 0 && (K % 64) == 0) /* K64 */
				{
					if (M + N < 2253 || M + K > 720|| N + K < 406 || M + N + K > 2796)
					{
						return clblasNotImplemented;
					}
				}
				else if ((M % 64) == 0 && (N % 64) == 0 && (K % 64) != 0) /* MN64 */	{}
				else if ((M % 64) == 0 && (N % 64) != 0 && (K % 64) == 0) /* MK64 */
				{
					if (M < 512 || N < 656 || K < 128 || M + N < 2895 || M + K > 1280 || N + K < 1424 || M + N + K < 3814)
					{
						return clblasNotImplemented;
					}
				}
				else if ((M % 64) != 0 && (N % 64) == 0 && (K % 64) == 0) /* NK64 */
				{
					if (M < 656 || N < 512 || M + N < 2908 || M + K < 958 || N + K < 1024 || M + N + K < 3663) {}
					else
					{
						return clblasNotImplemented;
					}
				}
				else if ((M % 64) == 0 && (N % 64) == 0 && (K % 64) == 0) /* MNK64 */
				{
					return clblasNotImplemented;
				}
				else
			...
			else if (transA == clblasNoTrans && transB == clblasTrans)
			{
				...
				{
					if (N > 1536 || M + N > 4108 || N + K > 4224 || M + N + K > 6192)
					{
						return clblasNotImplemented;
					}
				}
			...
			else if (transA == clblasTrans && transB == clblasTrans)
			{
				...
				{
					if (M + N > 4731 || N + K > 5120 || M + N + K > 7634)
					{
						return clblasNotImplemented;
					}
				}
		...
		else if (order == clblasRowMajor)
		{
			if (transA == clblasNoTrans && transB == clblasNoTrans)
			{
				...
				{
					if (M < 512 || N < 565 || M + N < 2942 || M + K < 1152 || N + K < 1423 || M + N + K < 3814)
					{
						return clblasNotImplemented;
					}
				}
				...
				{
					if (M > 656 || N < 512 || M + N < 2704 || M + K < 1424 || N + K < 1280 || M + N + K < 3710)
					{
						return clblasNotImplemented;
					}
				}
			...
			else if (transA == clblasNoTrans && transB == clblasTrans)
			{
				...
				{
					if (K < 128 || M + N > 4990 || M + K < 406 || N + K < 320 || M + N + K < 598 || M + N + K > 9551)
					{
						return clblasNotImplemented;
					}
				}
			...
			else if (transA == clblasTrans && transB == clblasNoTrans)
			{
				...
				{
					if (M > 1536 || M + N > 4070 || M + K < 192 || M + K > 4224 || M + N + K < 470 || M + N + K > 6192)
					{
						return clblasNotImplemented;
					}
				}
			...
			else if (transA == clblasTrans && transB == clblasTrans)
			{
				...
				{
					if (M > 2560 || M + N > 4731 || M + K > 5120 || M + N + K > 7634)
					{
						return clblasNotImplemented;
					}
				}
...
#endif
		unsigned int threadTile[2] = { 4, 4 };
		unsigned int groupSize[2] = { 8, 8 };
		if (transA == clblasNoTrans && transB == clblasNoTrans)
		{
			kernelSource = sgemm_NN_gfx900_tensile_src;
			clKernel = &sgemm_NN_gfx900_tensile_clKernel;
			kernelBinary = sgemm_NN_gfx900_tensile_bin;
			kernelBinarySize = sgemm_NN_gfx900_tensile_binSize;

			threadTile[0] = sgemm_NN_gfx900_tensile_microTileNumRows;
			threadTile[1] = sgemm_NN_gfx900_tensile_microTileNumCols;
			groupSize[0] = sgemm_NN_gfx900_tensile_workGroupNumRows;
			groupSize[1] = sgemm_NN_gfx900_tensile_workGroupNumCols;
		}
		else if (transA == clblasNoTrans && transB == clblasTrans)
		{
			kernelSource = sgemm_NT_gfx900_tensile_src;
			clKernel = &sgemm_NT_gfx900_tensile_clKernel;
			kernelBinary = sgemm_NT_gfx900_tensile_bin;
			kernelBinarySize = sgemm_NT_gfx900_tensile_binSize;

			threadTile[0] = sgemm_NT_gfx900_tensile_microTileNumRows;
			threadTile[1] = sgemm_NT_gfx900_tensile_microTileNumCols;
			groupSize[0] = sgemm_NT_gfx900_tensile_workGroupNumRows;
			groupSize[1] = sgemm_NT_gfx900_tensile_workGroupNumCols;
		}
		else if (transA == clblasTrans && transB == clblasNoTrans)
		{
			kernelSource = sgemm_TN_gfx900_tensile_src;
			clKernel = &sgemm_TN_gfx900_tensile_clKernel;
			kernelBinary = sgemm_TN_gfx900_tensile_bin;
			kernelBinarySize = sgemm_TN_gfx900_tensile_binSize;

			threadTile[0] = sgemm_TN_gfx900_tensile_microTileNumRows;
			threadTile[1] = sgemm_TN_gfx900_tensile_microTileNumCols;
			groupSize[0] = sgemm_TN_gfx900_tensile_workGroupNumRows;
			groupSize[1] = sgemm_TN_gfx900_tensile_workGroupNumCols;
		}
		else if (transA == clblasTrans && transB == clblasTrans)
		{
			kernelSource = sgemm_TT_gfx900_tensile_src;
			clKernel = &sgemm_TT_gfx900_tensile_clKernel;
			kernelBinary = sgemm_TT_gfx900_tensile_bin;
			kernelBinarySize = sgemm_TT_gfx900_tensile_binSize;

			threadTile[0] = sgemm_TT_gfx900_tensile_microTileNumRows;
			threadTile[1] = sgemm_TT_gfx900_tensile_microTileNumCols;
			groupSize[0] = sgemm_TT_gfx900_tensile_workGroupNumRows;
			groupSize[1] = sgemm_TT_gfx900_tensile_workGroupNumCols;
		}
		else
		{
			return clblasNotImplemented;
		...
		unsigned int sizeI = M;
		unsigned int sizeJ = N;
		unsigned int sizeK = 1; // ?
		unsigned int sizeL = K;

		unsigned int strideC1 = std::max(sizeI, ldc);
		unsigned int strideC2 = strideC1 * sizeJ;

		unsigned int strideA1L = sizeI;  // non-transposed
		unsigned int strideA2K = sizeI * sizeL;
		unsigned int strideA1I = sizeL;  // transposed
//		unsigned int strideA2K = sizeI * sizeL;
		unsigned int strideA1 = std::max(transA == clblasNoTrans ? sizeI : sizeL, lda);
		unsigned int strideA2 = strideA1 * (transA == clblasNoTrans ? sizeL : sizeJ);

		unsigned int strideB1J = sizeL;  // non-transposed
		unsigned int strideB2K = sizeL * sizeJ;
		unsigned int strideB1L = sizeJ;  // transposed
//		unsigned int strideB2K = sizeL * sizeJ;
		unsigned int strideB1 = std::max(transB == clblasNoTrans ? sizeL : sizeJ, ldb);
		unsigned int strideB2 = strideB1 * (transB == clblasNoTrans ? sizeJ : sizeL);

		/* index assignments */
		const unsigned int indexD0 = 0;
		const unsigned int indexD1 = 1;
		const unsigned int indexDU = 3;
		...
		globalWorkSize[0][0] = totalWorkGroups0*localWorkSize[0];
		globalWorkSize[0][1] = totalWorkGroups1*localWorkSize[1];

		/* offsets */
		unsigned int offsets[numKernels][1][3];
		offsets[0][0][0] = offC; // tensorC
		offsets[0][0][1] = offA; // tensorA
		offsets[0][0][2] = offB; // tensorB
...
//		sizes[0][0][3] = sizeL;
		...
		// kernel args same for all enqueues
		err = clSetKernelArg( *clKernel, 0, sizeof(cl_mem), &C ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 1, sizeof(cl_mem), &A ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 2, sizeof(cl_mem), &B ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 3, sizeof(float), &alpha ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 4, sizeof(float), &beta ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 5, sizeof(unsigned int), &offsets[0][0][0]); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 6, sizeof(unsigned int), &offsets[0][0][1]); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 7, sizeof(unsigned int), &offsets[0][0][2]); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 8, sizeof(unsigned int), &strideC1 ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 9, sizeof(unsigned int), &strideC2 ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 10, sizeof(unsigned int), &strideA1 ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 11, sizeof(unsigned int), &strideA2 ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 12, sizeof(unsigned int), &strideB1 ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 13, sizeof(unsigned int), &strideB2 ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 14, sizeof(unsigned int), &sizeI ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 15, sizeof(unsigned int), &sizeJ ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 16, sizeof(unsigned int), &sizeK ); CL_CHECK(err);
		err = clSetKernelArg( *clKernel, 17, sizeof(unsigned int), &sizeL ); CL_CHECK(err);
    ...
    bool &specialCaseHandled)
{
    ...
    if (gfxipMajor == GFXIP_MAJOR_VEGA && gfxipMinor == GFXIP_MINOR_VEGA_20)
    {
        ...
        // setup args
        err = clSetKernelArg(*clKernel,  0, sizeof(C), &C);                 CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  1, sizeof(A), &A);                 CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  2, sizeof(B), &B);                 CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  3, sizeof(alpha), &alpha);         CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  4, sizeof(beta),  &beta);          CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  5, sizeof(offC), &offC);           CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  6, sizeof(offA), &offA);           CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  7, sizeof(offB), &offB);           CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  8, sizeof(strideC1), &strideC1);   CL_CHECK(err);
        err = clSetKernelArg(*clKernel,  9, sizeof(strideC2), &strideC2);   CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 10, sizeof(strideA1), &strideA1);   CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 11, sizeof(strideA2), &strideA2);   CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 12, sizeof(strideB1), &strideB1);   CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 13, sizeof(strideB2), &strideB2);   CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 14, sizeof(sizeI), &sizeI);         CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 15, sizeof(sizeJ), &sizeJ);         CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 16, sizeof(sizeK), &sizeK);         CL_CHECK(err);
        err = clSetKernelArg(*clKernel, 17, sizeof(sizeL), &sizeL);         CL_CHECK(err);
