...
#ifndef KERNEL_SGEMM_COL_NT_B1_ML128_NL128_KX16_SRC_H
#define KERNEL_SGEMM_COL_NT_B1_ML128_NL128_KX16_SRC_H

const unsigned int sgemm_Col_NT_B1_ML128_NL128_KX16_workGroupNumRows = 16;
const unsigned int sgemm_Col_NT_B1_ML128_NL128_KX16_workGroupNumCols = 16;
const unsigned int sgemm_Col_NT_B1_ML128_NL128_KX16_microTileNumRows = 8;
const unsigned int sgemm_Col_NT_B1_ML128_NL128_KX16_microTileNumCols = 8;
const unsigned int sgemm_Col_NT_B1_ML128_NL128_KX16_unroll = 16;

static const char * const sgemm_Col_NT_B1_ML128_NL128_KX16_src ="\n"
"/* sgemm_Col_NT_B1_ML128_NL128_KX16 */\n"
"\n"
"/* kernel parameters */\n"
"#define WG_NUM_ROWS          16\n"
"#define WG_NUM_COLS          16\n"
"#define MICRO_TILE_NUM_ROWS  8\n"
"#define MICRO_TILE_NUM_COLS  8\n"
"#define MACRO_TILE_NUM_ROWS  128\n"
"#define MACRO_TILE_NUM_COLS  128\n"
...
"  rB[6] = localB[offB + 6*WG_NUM_COLS]; \\\n"
"  rB[7] = localB[offB + 7*WG_NUM_COLS]; \\\n"
...
"__kernel void sgemm_Col_NT_B1_ML128_NL128_KX16(\n"
...
"\n"
"  /* work item indices */\n"
"  uint groupRow = M / 128; // last row\n"
"  uint groupCol = N / 128; // last column\n"
...
"    lA[ 6*localAStride ] = ( globalARow(6) >= M) ? 0.0 : A[ GET_GLOBAL_INDEX_A( globalARow(6), globalACol(6) ) ];\n"
"    lA[ 7*localAStride ] = ( globalARow(7) >= M) ? 0.0 : A[ GET_GLOBAL_INDEX_A( globalARow(7), globalACol(7) ) ];\n"
...
"    lB[ 6*localBStride ] = ( globalBCol(6) >= N) ? 0.0 : B[ GET_GLOBAL_INDEX_B( globalBRow(6), globalBCol(6) ) ];\n"
"    lB[ 7*localBStride ] = ( globalBCol(7) >= N) ? 0.0 : B[ GET_GLOBAL_INDEX_B( globalBRow(7), globalBCol(7) ) ];\n"
"    barrier(CLK_LOCAL_MEM_FENCE);\n"
"    uint offA = localRow;\n"
"    uint offB = localCol;\n"
"\n"
...
"  if (globalCRow+0*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+0*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[0][6], beta )}\n"
"  if (globalCRow+0*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+0*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[0][7], beta )}\n"
...
"  if (globalCRow+1*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+1*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[1][6], beta )}\n"
"  if (globalCRow+1*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+1*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[1][7], beta )}\n"
...
"  if (globalCRow+2*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+2*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[2][6], beta )}\n"
"  if (globalCRow+2*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+2*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[2][7], beta )}\n"
...
"  if (globalCRow+3*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[3][6], beta )}\n"
"  if (globalCRow+3*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+3*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[3][7], beta )}\n"
...
"  if (globalCRow+4*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[4][6], beta )}\n"
"  if (globalCRow+4*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+4*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[4][7], beta )}\n"
...
"  if (globalCRow+5*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[5][6], beta )}\n"
"  if (globalCRow+5*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+5*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[5][7], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+0*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+0*WG_NUM_COLS) ], alpha, rC[6][0], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+1*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+1*WG_NUM_COLS) ], alpha, rC[6][1], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+2*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+2*WG_NUM_COLS) ], alpha, rC[6][2], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+3*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[6][3], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+4*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[6][4], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+5*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[6][5], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[6][6], beta )}\n"
"  if (globalCRow+6*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+6*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[6][7], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+0*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+0*WG_NUM_COLS) ], alpha, rC[7][0], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+1*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+1*WG_NUM_COLS) ], alpha, rC[7][1], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+2*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+2*WG_NUM_COLS) ], alpha, rC[7][2], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+3*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+3*WG_NUM_COLS) ], alpha, rC[7][3], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+4*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+4*WG_NUM_COLS) ], alpha, rC[7][4], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+5*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+5*WG_NUM_COLS) ], alpha, rC[7][5], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+6*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+6*WG_NUM_COLS) ], alpha, rC[7][6], beta )}\n"
"  if (globalCRow+7*WG_NUM_ROWS < M)  if (globalCCol+7*WG_NUM_COLS < N){  TYPE_MAD_WRITE( C[ GET_GLOBAL_INDEX_C( globalCRow+7*WG_NUM_ROWS, globalCCol+7*WG_NUM_COLS) ], alpha, rC[7][7], beta )}\n"
"\n"
"}\n"
"";
