...
def processAllKernelParameterCombinations(argv):
  ...
  # create directories
  kernelSourcePath = "GemmKernelSources"
  kernelBinaryPath = "GemmKernelBinaries"
  ...
  kernelSelectionLogic = KernelSelectionLogic();
      ...
      if microTileSize[0]*microTileSize[1] <= microTileMaxProduct:
        for workGroupDim in listWorkGroupDims:
          for unroll in listUnroll:
            ...
            kernel.microTileNumCols = microTileSize[1]
...
class GemmOpenCLKernelSource:
  ...
  def makeKernelString(self):
    ...
    if self.precision=="s" or self.precision=="d":
      # real arithmetic
      self.ks += "#define TYPE_MAD(MULA,MULB,DST) DST = mad(MULA,MULB,DST);" + self.el
      if self.beta==1:
      ...
      # complex arithmetic
      if self.transA!="C" and self.transB!="C":
        # neither conjugate
        self.ks += (
          "#define TYPE_MAD(MULA,MULB,DST) \\\\" + self.el +
          "  DST.s0 = mad(  MULA.s0, MULB.s0, DST.s0 ); \\\\" + self.el +
          "  DST.s0 = mad( -MULA.s1, MULB.s1, DST.s0 ); \\\\" + self.el +
          "  DST.s1 = mad(  MULA.s0, MULB.s1, DST.s1 ); \\\\" + self.el +
          "  DST.s1 = mad(  MULA.s1, MULB.s0, DST.s1 );" + self.el )
      elif self.transA=="C" and self.transB!="C":
...
class KernelSourceIncludes:
  ...
  ##############################################################################
  def __init__(self):
    self.fileName = "GemmSourceIncludes.h"
...
class KernelSourceBuildOptions:
  ...
  ##############################################################################
  def __init__(self):
    self.fileName = "GemmKernelSourceBuildOptions.h"
...
class KernelBinaryBuildOptions:
  ...
  ##############################################################################
  def __init__(self):
    self.fileName = "GemmKernelBinaryBuildOptions.h"
...
class CppKernelEnumeration:
  ...
  ##############################################################################
  def __init__(self):
    self.fileName = "GemmKernelEnumeration.h"
    self.fileStr = ""
    self.count = 0
    self.precision = ""
...
class KernelSelectionLogic:
  ...
  def __init__(self):
    ...
    self.kernelSelectionFileName = "GemmKernelSelection.h"
